...
public class connect {
	...
	public int mysql(String IDuser, String IDpass) {
		...
		try {
			...
			pst = (PreparedStatement) conn
					.prepareStatement("select username, password, accType from login_info where username=?");
	...
	public int register(String UserID, String UserPass, String Mail, String FName, String LName, String birthday) {
		...
		try {
			...
			pst = (PreparedStatement) conn.prepareStatement(
					"insert into login_info(username, password, accType, email, firstname, lastname, joined, birthday) "
							+ "values (?, ?, ?, ?, ?, ?, ?, ?);");
	...
	public ResultSet readInfo(String UserName) {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement(
					"select firstname, lastname, email, birthday, password from login_info where username=?");
			//// Set the first element with username
	...
	public boolean editUser(String UserID, String UserPass, String Mail, String birthday) {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn
					.prepareStatement("update login_info set password=?, email=?, birthday=?" + "where username=?");
	...
	public ResultSet getUser() {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement(
					"select username,password, firstname, lastname, joined from ieltsonline.login_info where accType='1'");
			// Fill data take form SQL to resultset
			result = pst.executeQuery();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int saveResult(String examination_code, String username, int reading, int listening, String date_made) {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement(
					"insert into my_result(examination_code, username, listening, reading, total, date_made) "
							+ "values (?, ?, ?, ?, ?, ?);");
			// Create a variable
			int total = 0;
	...
	public int saveTest(String examination_code, String username) {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement(
					"INSERT INTO ieltsonline.practice_read (examination_code, username, date_added, times)"
							+ " values (?, ?, ?, ?);");
			DateFormat df = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
			java.util.Date date = Calendar.getInstance().getTime();
			String reportDate = df.format(date);
	...
	public int editResult(String examination_code, String username, int listening, int reading, String date_made) {
		...
		try {
			...
			PreparedStatement pst = null;
			pst = (PreparedStatement) conn
					.prepareStatement("update my_result set listening=?, reading=?, total=?, date_made=? "
	...
	public int updateData(String examination_code, String username, String type) {
		...
		try {
			...
			pst2 = (PreparedStatement) conn
					.prepareStatement("select times from " + type + " " + "where username=? and examination_code=?");
			// Set the first element with username
			...
			// Set value to query
			pst = (PreparedStatement) conn
					.prepareStatement("update " + type + " set times=? " + "where username=? and examination_code=?");
	...
	public ResultSet getFullTest(String UserName) {
		...
		try {
			...
			PreparedStatement pst = null;
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement("SELECT * FROM ieltsonline.practice_fulltest;");
			// Fill data take form SQL to resultset
			result = pst.executeQuery();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public ResultSet getReadTest(String UserName) {
		...
		try {
			...
			PreparedStatement pst = null;
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement("SELECT * FROM ieltsonline.practice_read;");
			// Fill data take form SQL to resultset
			result = pst.executeQuery();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public ResultSet getListenTest(String UserName) {
		...
		try {
			...
			PreparedStatement pst = null;
			// Set value to query
			pst = (PreparedStatement) conn.prepareStatement("SELECT * FROM ieltsonline.practice_listen;");
			// Fill data take form SQL to resultset
			result = pst.executeQuery();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public boolean deleteFullTest(String UserID, String examCode) {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn
					.prepareStatement("DELETE FROM ieltsonline.practice_fulltest WHERE examination_code=?;");
			// Set the first element with examination code
	...
	public boolean deleteListenTest(String UserID, String examCode) {
		...
		try {
			...
			// Set value to query
			pst = (PreparedStatement) conn
					.prepareStatement("delete from practice_listen where and examination_code=? ;");
			// Set the first element with username
			pst.setString(1, UserID);
